...
const Signup: React.FC<SignupProps> = ({ user, db }) => {
  ...
  useEffect(() => {
    // If no inviteId, skip validation and allow signup
    if (!inviteId) {
      setValidating(false);
  ...
  const handleUserCreated = async (newUser: User, displayNameValue?: string) => {
    ...
    // If there's a valid invite, add user to that club
    if (inviteValid && inviteData?.clubId && inviteId) {
      ...
      // No invite - just create account and go to profile
      navigate("/profile");
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    // If there's an invite, validate it first
    if (inviteId) {
      if (!inviteValid || !inviteData?.clubId) {
    ...
    try {
      ...
      if (inviteId && inviteData?.email && newUser.email?.toLowerCase() !== inviteData.email.toLowerCase()) {
            ...
            <>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <p style={{
                        ...
                        Please contact the person who invited you for a new invite link, or sign up without an invite.
                      </p>
                    </div>
                  </div>
                </>
              ...
              <div className="login-header">
                ...
                <p className="login-subtitle">
                  {inviteId && inviteValid && inviteData?.clubName 
                    ? `Join ${inviteData.clubName}`
                    : inviteId && inviteData?.inviterName
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      ...
                      {inviteId && inviteValid ? 'Create Account & Join Club' : 'Create Account'}
                    </>
                  )}
                </button>
              </form>
